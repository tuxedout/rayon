...
#ifndef RAYON_CPP_CMAKE_SCHUM_H
#define RAYON_CPP_CMAKE_SCHUM_H

#include "../RayOn/plus-d/RTSceneCreator.h"
#include "../RayOn/Materials/ReflectingMaterial.h"
#include "../RayOn/Color.h"
...
namespace RN {
    ...
    class SCHUM: public RTSceneCreator{
    public:
        void initScene(RN::RTScene &scene, RN::RTScene &lights, Camera &camera) override {

            RN::Materials::DiffusiveMaterial mat_light(RN::vec3d(2.8, 2.75, 2.482), RN::vec3d(0.83, 0.83, 0.83));
            RN::Materials::DiffusiveMaterial mat_light_lines(std::make_shared<RN::Textures::LinesTexture>(RN::Textures::LinesTexture(vec3d(0.9), vec3d(0.0), 9, false)), std::make_shared<RN::Textures::LinesTexture>(RN::Textures::LinesTexture(vec3d(0.2, 0.6, 1.1), vec3d(0.0), 9,
                                                                                                                                                                                                                                                 true)));
            ...
            RN::Materials::SolidColorMaterial lines2(std::make_shared<RN::Textures::LinesTexture>(RN::Textures::LinesTexture(vec3d(0.0), vec3d(1.0), 100,
                                                                                                                             true)));
            ...
            RN::Materials::RefractingMaterial  mat_refr(RN::vec3d(1.0, 0.8, 0.30));
            ...
            mat_blue_reflecting.reflectivity = 0.95;
            mat_blue_reflecting.roughness = 0.8;
...
//            mats.push_back(std::make_shared<RN::Materials::SolidColorMaterial>(texture_material));
            ...
            RN::vec3d box_origin = RN::vec3d(0,6.2,0);

            const int grid_size = 20;
            const double grid_spacing = 1.9;

            RN::Grid3D<double> grid(grid_size, grid_size, 1, grid_spacing, box_origin + vec3d(-grid_size*grid_spacing/2, -grid_size*grid_spacing/2-7,-25));

            const double box_size = 1.7;

            RN::Interval<double> interval0_07(-3*box_size/2,4*box_size/2);
...
//                if (tmp_clr.x < 0.5) {
                    ...
                    RN::RTBox box(c + vec3d(0,sin((i/grid_size)*2 - 1),interval0_07.getRandom()), box_size, mat_p);
...
//            RN::RTSphere sphere2(box_origin + RN::vec3d(0, 1350, 300.0), 300.0,std::make_shared<RN::Materials::DiffusiveMaterial>(mat_light));
//            scene.add(std::make_shared<RN::RTSphere>(sphere2));
//            lights.add(std::make_shared<RN::RTSphere>(sphere2));

            RN::vec3d cam_center =  vec3d(0, 3.25, 21);//RN::vec3d(box_origin.x + 280,box_origin.y + 330, box_origin.z + 710);

            std::string asspth = ASSETS_PATH;

//            auto mesh = RN::RTMesh::loadFromOBJ(asspth + "box.obj", std::make_shared<RN::Materials::Reflecting>(mat_gold_reflecting));
            auto mesh = RN::RTMesh::loadFromOBJ(asspth + "schum.obj", std::make_shared<RN::Materials::ReflectingMaterial>(mat_yellow_reflecting));
