...
#ifndef RAYON_CPP_CMAKE_OBJ_TEST_H
#define RAYON_CPP_CMAKE_OBJ_TEST_H
...
namespace RN {
    ...
    class ObjTest: public RTSceneCreator{
    public:
...
//            mats.push_back(std::make_shared<RN::Materials::SolidColorMaterial>(texture_material));
            ...
            const double grid_spacing = 0.47;

            RN::Grid3D<double> grid(grid_size, grid_size, 1, grid_spacing, box_origin + vec3d(-(grid_size-1)*grid_spacing/2, -(grid_size-1)*grid_spacing/2,-23));

            const double box_size = 0.23;

            RN::Interval<double> interval0_07(-10*box_size/2,10*box_size/2);

//            grid.generateGrid([&scene, &materials, &image_shared_ptr, box_size, &interval0_07](int i, int k, int j, RN::vec3d c, RN::vec3d rc) {
//                auto tmp_clr = image_shared_ptr->color({rc.x, 1.0 - rc.y}, {});
////                if (tmp_clr.x < 0.5) {
//                    RN::RTBox box(c + vec3d(0,0,interval0_07.getRandom()*c.length()), box_size, materials.getRandom());
//                    scene.add(std::make_shared<RN::RTBox>(box));
////                }
//            });

            RN::RTSphere sphere2(box_origin + RN::vec3d(0, 1350, 700.0), 800.0,std::make_shared<RN::Materials::DiffusiveMaterial>(mat_light));
            scene.add(std::make_shared<RN::RTSphere>(sphere2));

            RN::vec3d cam_center =  vec3d(17, 3, 19.5);//RN::vec3d(box_origin.x + 280,box_origin.y + 330, box_origin.z + 710);
...
#endif // RAYON_CPP_CMAKE_OBJ_TEST_H