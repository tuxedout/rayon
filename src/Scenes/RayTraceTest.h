...
#ifndef RAYON_CPP_CMAKE_RAYTRACETEST_H
#define RAYON_CPP_CMAKE_RAYTRACETEST_H
...
namespace RN {
    ...
    class RayTraceTest: public RTSceneCreator{
    public:
        void initScene(RN::RTScene &scene, RN::RTScene &lights, Camera &camera) override {

            RN::Materials::DiffusiveMaterial mat_light(RN::vec3d(5.8, 5.75, 5.482), RN::vec3d(0.3, 0.23, 0.3));
...
//            mats.push_back(std::make_shared<RN::Materials::SolidColorMaterial>(texture_material));
            ...
            const double box_size = 2.7;

            RN::Interval<double> interval0_07(-3*box_size/2,4*box_size/2);

            grid.generateGrid([&scene, &materials, &image_shared_ptr, box_size, &interval0_07, &mat_yellow](int i, int k, int j, RN::vec3d c, RN::vec3d rc) {
                auto tmp_clr = image_shared_ptr->color({rc.x, 1.0 - rc.y}, {});
//                if (tmp_clr.x < 0.5) {
                    RN::RTBox box(c + vec3d(0,0,interval0_07.getRandom()), box_size, std::make_shared<Materials::SolidColorMaterial>(mat_yellow));
                    scene.add(std::make_shared<RN::RTBox>(box));
//                }
            });

            RN::RTSphere sphere2(box_origin + RN::vec3d(0, 1350, 1000.0), 800.0,std::make_shared<RN::Materials::DiffusiveMaterial>(mat_light));
            scene.add(std::make_shared<RN::RTSphere>(sphere2));

            RN::vec3d cam_center =  vec3d(0, 3, 11);//RN::vec3d(box_origin.x + 280,box_origin.y + 330, box_origin.z + 710);

            std::string asspth = ASSETS_PATH;
//
//            RN::RTCylinder cyl({},1,4, std::make_shared<RN::Materials::Refracting>(mat_refr));
//            scene.add(std::make_shared<RN::RTCylinder>(cyl));
...
#endif // RAYON_CPP_CMAKE_RAYTRACETEST_H