...
#ifndef RAYON_CPP_CMAKE_TEXTURE_TEST_H
#define RAYON_CPP_CMAKE_TEXTURE_TEST_H
...
namespace RN {
    ...
    class TextureTest: public RTSceneCreator{
    public:
        void initScene(RN::RTScene &scene, RN::RTScene &lights, Camera &camera) override {

            RN::Materials::DiffusiveMaterial mat_light(RN::vec3d(0.8, 0.75, 0.482), RN::vec3d(1.3, 1.23, 1.3));
            ...
            RN::Materials::DiffusiveMaterial mat_light22(
                    ...
                    std::make_shared<RN::Textures::CheckeredTexture>(
                            RN::Textures::CheckeredTexture(vec3d(1.40, 0.4, 0.4),
            ...
            auto opt_image = RN::Textures::ImageTexture::Load(assets_path + "tux_32_9.png");
            if (opt_image) {
...
//            mats.push_back(std::make_shared<RN::Materials::Diffusive>(mat_light3));
...
//            mats.push_back(std::make_shared<RN::Materials::SolidColorMaterial>(texture_material));
            ...
            RN::vec3d cam_center = RN::vec3d(box_origin.x + 310,box_origin.y + 310, box_origin.z + 310);

//            RN::Grid3D<double> grid(32, 9, 5, 10, box_origin + vec3d(-100,0,-25));
//
//            grid.generateGrid([&scene, &materials, &image_shared_ptr](int i, int k, int j, RN::vec3d c, RN::vec3d rc) {
//                auto tmp_clr = image_shared_ptr->color({rc.x, rc.y}, {});
//                if (tmp_clr.x < 0.5) {
//                    RN::RTBox box(c, 9, materials.getRandom());
//                    scene.add(std::make_shared<RN::RTBox>(box));
//                }
//            });

            RN::RTSphere sphere2(box_origin + RN::vec3d(50, 2550, 50), 2100.0,std::make_shared<RN::Materials::DiffusiveMaterial>(mat_light2));
            ...
            RN::RTQuad quadYX( box_origin + vec3d(0,0,-5), RN::vec3d(0,80, 0), RN::vec3d(80, 0, 0), std::make_shared<RN::Materials::DiffusiveMaterial>(mat_light));
            scene.add(std::make_shared<RN::RTQuad>(quadYX));

            RN::RTQuad quadYX2( box_origin + vec3d(0,90,-5), RN::vec3d(0,80, 0), RN::vec3d(80, 0, 0), std::make_shared<RN::Materials::SolidColorMaterial>(mat_white));
            scene.add(std::make_shared<RN::RTQuad>(quadYX2));

            RN::RTQuad quadXZ( box_origin + vec3d(0,-5,0), RN::vec3d(80,0, 0), RN::vec3d(0, 0, 80), std::make_shared<RN::Materials::SolidColorMaterial>(texture_material));
            scene.add(std::make_shared<RN::RTQuad>(quadXZ));

//            RN::RTSphere sphere(box_origin + 45 + vec3d(0, 100, 0), 30, std::make_shared<RN::Materials::SolidColorMaterial>(texture_material));
//            scene.add(std::make_shared<RN::RTSphere>(sphere));

            RN::RTEllipsoid elli(box_origin + vec3d(40, 5, 40), {17,2,17}, std::make_shared<RN::Materials::RefractingMaterial>(mat_refr));
            scene.add(std::make_shared<RN::RTEllipsoid>(elli));

            RN::RTEllipsoid elli2(box_origin + vec3d(40, 70, 40), {10,1,10}, std::make_shared<RN::Materials::RefractingMaterial>(mat_refr));
            scene.add(std::make_shared<RN::RTEllipsoid>(elli2));

            RN::RTEllipsoid elli3(box_origin + vec3d(40, 150, 40), {20,1,20}, std::make_shared<RN::Materials::RefractingMaterial>(mat_refr));
            scene.add(std::make_shared<RN::RTEllipsoid>(elli3));

            camera.setCenter(cam_center);
            camera.lookAt(box_origin + vec3d(0, 50, 0));
        };
    };

} // RN

#endif // RAYON_CPP_CMAKE_TEXTURE_TEST_H