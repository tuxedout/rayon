...
void scene_update(float x,float y){
    r1.sd = circle(x, y, 0.5*u_resolution.x, 0.5*u_resolution.y, 35.0);
    //ssDot r2 = ssDot(circle(x,y,m.x,m.y, 0.25),  2.0, 1.2,vec3(0.7, 0.1, 0.5), vec3(0.8,0.6,0.0));
    r2.sd = circle(x,y,m.x,m.y, 20);
    r3.sd = circle(x,y,0.1*u_resolution.x,0.1*u_resolution.y, 25);
    r4.sd = box(x,y,u_resolution.x-250.0,u_resolution.y-100.0,vec2(200,80));
    r5.sd = circle(x,y,0.3*u_resolution.x,0.1*u_resolution.y, 45.0);
    r6.sd = circle(x,y, 0.7*u_resolution.x,0.6*u_resolution.y, 25);
    r7.sd = circle(x,y, 0.2*u_resolution.x,0.5*u_resolution.y, 25);
    r8.sd = box(x,y,0.4*u_resolution.x,0.7*u_resolution.y,vec2(25,50));
    r9.sd = box(x,y,0.4*u_resolution.x,0.1*u_resolution.y,vec2(50,25));
}
...
void main(){
    vec2 uv = fragTexCoord * u_resolution;

    m  = (u_mouse.xy);

    m.y = u_resolution.y - m.y;

    ssDot sv = scene(uv.x, uv.y);

    //vec2 n = calculateNormal(uv);
    //vec3 col = mats[sv.mat_idx].emission;
