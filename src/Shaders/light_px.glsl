...
vec2 calculateNormal(vec2 p) {
    ...
    // Градиенты по X и Y
    float gx = (getSDF(p + dx) - getSDF(p - dx));
    float gy = (getSDF(p + dy) - getSDF(p - dy));

    // Диагональные градиенты
    float gxy = (getSDF(p + dxy) - getSDF(p - dxy));
    float gyx = (getSDF(vec2(p.x + dx.x, p.y - dy.y)) - getSDF(vec2(p.x - dx.x, p.y + dy.y)));
